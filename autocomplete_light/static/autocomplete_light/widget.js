...
yourlabs.Widget = function(widget) {
    ...
    // Edit by Aleksandr Pasevin
    // Redesign already inserted choices on init
    for (var i = this.deck.length - 1; i >= 0; i--) {
        var $currDeck = $(this.deck[i]);
        var hasChildren = $currDeck.children().length > 0 ? true : false;
        if (hasChildren) {
            var value = $currDeck.children().attr('data-value');
            var text  = $currDeck.children().text();
            var newLookChoice = $('<div data-value="'+value+'" class="input-group"><input type="text" class="form-control" value="'+text+'" disabled></div>');
            this.deck.html(newLookChoice);
        }
    };
...
yourlabs.Widget.prototype.optionChoice = function(option) {
    ...
    var target = optionChoice.find('.prepend-option-html');

    if (target.length) {
        target.prepend(option.html());
    } else {
...
yourlabs.Widget.prototype.addRemove = function(choices) {
    ...
    var target = choices.find('.append-remove');

    if (target.length) {
        target.append(removeTemplate);
    } else {
        // Add the remove icon to each choice
        choices.append(removeTemplate);
    } 
}
...
yourlabs.Widget.prototype.addToDeck = function(choice, value) {
    ...
    if (!existing_choice.length) {
        ...
        // Edit by Aleksandr Pasevin
        // Redesign before appending choice element
        var newLookChoice = $('<div data-value="'+value+'" class="input-group"><input type="text" class="form-control" value="'+$(choice).text()+'" disabled></div>');

        var deckChoice = this.deckChoiceHtml(newLookChoice);
